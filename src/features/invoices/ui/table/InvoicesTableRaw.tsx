...
const ItemRaw: FC<{ invoice: Invoice; style: React.CSSProperties; onCancel: (id: string) => Promise<void>; isCancelLoading: boolean }> = ({
    invoice,
    style,
    onCancel,
    isCancelLoading
    ...
    const { onCopy: onCopyPaidBy, hasCopied: hasCopiedPaidBy } = useClipboard(
        invoice.status === 'success' ? invoice.paidBy.toString() : ''
    );
        ...
        <>
            ...
            >
                ...
                <Td minW="160px" h={rawHeight} maxH={rawHeight} boxSizing="content-box">
                    {invoice.amount.toStringCurrencyAmount({ decimalPlaces: null })}
                </Td>
                <Td
                    minW="180px"
                    maxW="180px"
                ...
                >
                    <Flex align="center" justify="space-between" gap="2">
                        ...
                        <Flex gap="2" minW="fit-content">
                            {!!invoice.overpayment && (
                                <InvoiceOverpayment invoice={invoice} />
                            )}
                            {canCancel && (
...
const InvoicesTableRaw: FC<{ index: number; style: React.CSSProperties }> = ({ index, style }) => {
    const context = useContext(InvoicesTableContext);

    if (!context.invoices || index >= context.invoices.length) {
        return <LoadingRaw style={style} />;
    }

    const invoice = context.invoices[index];
    return (
        <ItemRaw
            key={invoice.id}
            style={style}
            ...
            isCancelLoading={context.isCancelLoading}
        />
    );
};

export default InvoicesTableRaw;
